...
var inserted = false;
	PubSub.subscribe("scrollTop", function(msg, data) {
        ...
        if(scrollTop > slide.top - geometry.height && scrollTop < slide.top + slide.height ) {
			...
			if(!inserted) {
				// var newItem= document.createElement('filter');
				// newItem.innerHTML= '<filter id="this_image" x="0%" y="0%" width="100%" height="100%"><feImage xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href=" 	"></feImage>			</filter>';
				// var x = document.getElementById("expanding").lastChild;
				// var list = document.getElementById("myList");
				// x.insertBefore(newItem, x.childNodes[0]); 
				// x.lastChild.setAttribute('fill','url(#this_image)')
				svg.append("text")
				.attr("x",260)
				.attr("y",320)
				.attr("fill","white")
			...
			f.setAttribute("opacity", cr(data)/200);
			f.setAttribute("font-size", cr(data)+'px');
