...
// Copyright 2009-2017 We Also Walk Dogs. All rights reserved.
...
forms.contextRoot = 'http://letsfreecongress.org/context/';

/* Any form initialization we can do before we have context */
forms.beforeContextLoad = function() {
	if (ak.form.event_search_on_load) {
		actionkit.forms.eventSearch(undefined, {"all": 1, "akid": ak.args.akid || '', "page": ak.form.page.value, "callback": "actionkit.forms.onEventSearchResults"})
	}
    ...
    if ( ak.args.zip 
         && ak.form.template 
         && ak.form.template.value == 'http://letsfreecongress.org/event_search.html'
         && !ak.form.have_events ) {
...
forms.loadContext = function() {
    ...
    if ( !/\/$/.test(root) ) root += 'http://letsfreecongress.org/';
...
forms.loadPrefiller = function() {
    var prefillerUrl = forms.contextRoot.replace('http://letsfreecongress.org/context/', 'http://letsfreecongress.org/samples/prefill.js');
    forms.createScriptElement(prefillerUrl);
}

forms.loadProgress = function() {
    var progressUrl = forms.contextRoot.replace(
        'http://letsfreecongress.org/context/', 
...
forms.loadText = function() {
    ...
    var textUrl = forms.contextRoot.replace('http://letsfreecongress.org/context/', relative_url);
    ak.textLoading = 1;
...
forms.handleQueryStringErrors = function() {
    ...
    if ( ak.form ) {
        $(ak.form).show();
        $(ak.form).closest('fieldset, .ak-field-box').show();
    }
...
forms.onValidationErrors = function(errors, field) {
    ...
    function getErrorBox(error) {
        ...
        var fieldName = error.split(':')[0],
            inputElem = ak.form[fieldName];
        if (!inputElem && actionkit.donations && actionkit.donations.vzero) {
            // v.zero workarounds
            if (fieldName == 'card_num') {
                inputElem = ak.form.querySelector("#ak-card_num-hosted");
            } else if (fieldName == 'card_code') {
                inputElem = ak.form.querySelector("#ak-card_code-hosted");
            } else if (fieldName == 'card_exp' || fieldName == 'card_exp_month' || fieldName == 'card_exp_year') {
                inputElem = ak.form.querySelector("#ak-exp_date-hosted");
            }
        }
        var input = inputElem && $(inputElem);
        return input && input.closest('.ak-err-above,.ak-err-below,.ak-err-after');
    }
    for ( error in errors ) {
    ...
    error_positions.forEach(function(pos) {
        $('.ak-errs-' + pos + ' :input:not([type="hidden"])').each( function () { 
            ...
            if ( $this.closest('.ak-err-' + pos).length == 0 ) {
                $this.parent().addClass('ak-err-' + pos)
            }
        } );
    })
...
// means recaptcha). Lets us, or folks monkeypatching, only run async
// code when it's really needed.
